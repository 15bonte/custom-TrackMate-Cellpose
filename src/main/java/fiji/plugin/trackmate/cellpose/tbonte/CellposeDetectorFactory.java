...
public class CellposeDetectorFactory<T extends RealType<T> & NativeType<T>> implements SpotGlobalDetectorFactory<T> {
	...
	/**
	 * The key to the parameter that stores the path to the Python instance that can
	 * run cellpose if you installed it via Conda or the cellpose executable if you
	 * have installed the standalone version. Something like
	...
	/**
	 * The key to the parameter that stores the path to the custom model file to use
	 * with Cellpose. It must be an absolute file path.
	 */
	...
	/**
	 * The key to the parameter that stores the second optional channel to segment.
	 * Use -1 to ignore.
	 */
	...
	/**
	 * The key to the parameter that store the estimated cell diameter. Contrary to
	 * Cellpose, this must be specified in physical units (e.g. µm) and TrackMate
	 * wil do the conversion. Use 0 or a negative value to have Cellpose determine
	 * this automatically (but it will take a bit longer).
	 */
	...
	/**
	 * Cellpose cellprob threshold parameter.
	 */
	public static final String KEY_CELLPROB_THRESHOLD = "CELLPROB_THRESHOLD";

	public static final Double DEFAULT_CELLPROB_THRESHOLD = Double.valueOf(0.0);

	/**
	 * Cellpose augment threshold parameter.
	 */
	public static final String KEY_AUGMENT = "AUGMENT";

	public static final Boolean DEFAULT_AUGMENT = Boolean.valueOf(false);

	/**
	 * They key to the parameter that configures whether Cellpose will try to use
	 * GPU acceleration. For this to work, a working Cellpose with working GPU
	 * support must be present on the system. If not, Cellpose will default to using
	 * the CPU.
	 */
	...
	/**
	 * The key to the parameter that stores the logger instance, to which Cellpose
	 * messages wil be sent. Values must be implementing {@link Logger}. This
	 * parameter won't be serialized.
	 * 
	 */
	...
	public static final String INFO_TEXT = "<html>" + "This detector relies on cellpose to detect objects." + "<p>"
			...
			+ "<p>" + "You will also need to specify the path to the <b>Python executable</b> that can run cellpose "
			...
			+ "this on Windows: 'C:\\Users\\tinevez\\Applications\\cellpose.exe'. " + "<p>"
			...
			+ "Cellpose: a generalist algorithm for cellular segmentation. " + "Nat Methods 18, 100–106 (2021)</a>"
			+ "<p>" + "Documentation for this module "
			+ "<a href=\"https://imagej.net/plugins/trackmate/trackmate-cellpose\">on the ImageJ Wiki</a>." + "</html>";
	...
	public SpotGlobalDetector<T> getDetector(final Interval interval) {
		...
		final double cellprobThreshold = (double) settings.get(KEY_CELLPROB_THRESHOLD);

		final boolean augment = (boolean) settings.get(KEY_AUGMENT);

		final CellposeSettings cellposeSettings = CellposeSettings.create().cellposePythonPath(cellposePythonPath)
				.customModel(customModelPath).model(model).channel1(channel).channel2(channel2).diameter(diameter)
				.useGPU(useGPU).simplifyContours(simplifyContours).flowThreshold(flowThreshold)
				.cellprobThreshold(cellprobThreshold).augment(augment).get();
	...
	public boolean forbidMultithreading() {
		/*
		 * We want to run one frame after another, because the inference for one frame
		 * takes all the resources anyway.
		 */
		return true;
	}
	...
	public boolean marshall(final Map<String, Object> settings, final Element element) {
		...
		ok = ok && writeAttribute(settings, element, KEY_CELLPROB_THRESHOLD, Double.class, errorHolder);
		ok = ok && writeAttribute(settings, element, KEY_AUGMENT, Boolean.class, errorHolder);
	...
	public boolean unmarshall(final Element element, final Map<String, Object> settings) {
		...
		ok = ok && readDoubleAttribute(element, settings, KEY_CELLPROB_THRESHOLD, errorHolder);
		ok = ok && readBooleanAttribute(element, settings, KEY_AUGMENT, errorHolder);
	...
	public Map<String, Object> getDefaultSettings() {
		...
		settings.put(KEY_CELLPROB_THRESHOLD, DEFAULT_CELLPROB_THRESHOLD);
		settings.put(KEY_AUGMENT, DEFAULT_AUGMENT);
	...
	public boolean checkSettings(final Map<String, Object> settings) {
		...
		ok = ok & checkParameter(settings, KEY_CELLPROB_THRESHOLD, Double.class, errorHolder);
		ok = ok & checkParameter(settings, KEY_AUGMENT, Boolean.class, errorHolder);
		...
		if (loggerObj != null && !Logger.class.isInstance(loggerObj)) {
			errorHolder.append("Value for parameter " + KEY_LOGGER + " is not of the right class. " + "Expected "
					+ Logger.class.getName() + ", got " + loggerObj.getClass().getName() + ".\n");
			ok = false;
		}

		final List<String> mandatoryKeys = Arrays.asList(KEY_CELLPOSE_PYTHON_FILEPATH, KEY_CELLPOSE_MODEL,
				KEY_TARGET_CHANNEL, KEY_OPTIONAL_CHANNEL_2, KEY_CELL_DIAMETER, KEY_USE_GPU, KEY_SIMPLIFY_CONTOURS,
				KEY_FLOW_THRESHOLD, KEY_CELLPROB_THRESHOLD, KEY_AUGMENT);
		final List<String> optionalKeys = Arrays.asList(KEY_CELLPOSE_CUSTOM_MODEL_FILEPATH, KEY_LOGGER);
